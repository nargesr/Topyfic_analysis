...
rule run_single_train:
    ...
    log:
        f"{config['workdir']}/{{name}}/{{n_topic}}/train/run_single_train_{{name}}_{{n_topic}}_{{random_state}}.log",
    run:
...
rule run_train_model:
    ...
    params:
        ...
        n_topic=lambda wildcards: wildcards.n_topic,
    log:
        f"{config['workdir']}/{{name}}/{{n_topic}}/train/run_train_model_{{name}}_{{n_topic}}_{{random_state}}.log",
    run:
