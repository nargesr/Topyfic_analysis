...
rule run_top_model:
    ...
    log:
        f"{config['workdir']}/{{name}}/{{n_topic}}/topmodel/run_top_model_{{name}}_{{n_topic}}.log",
    run:
...
rule run_analysis:
    ...
    params:
        ...
        n_topic=lambda wildcards: wildcards.n_topic,
    log:
        f"{config['workdir']}/{{name}}/{{n_topic}}/topmodel/run_analysis_{{name}}_{{n_topic}}.log",
    run:
...
if 'merge' in config:
    rule run_multiple_top_model:
        ...
        params:
            ...
            n_topic=lambda wildcards: wildcards.n_topic,
        log:
        f"{config['workdir']}/run_multiple_top_model.log"
        run:
            df, res = make_topmodel.find_best_n_topic(n_topics=config["n_topics"],
                names=config["names"],
